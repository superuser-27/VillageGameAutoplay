function on_msg_receive (msg)
      if msg.out or msg.from.print_name==msg.to.print_name then
		...
		file=io.open("VillageGameAutoplay_cp/setting.json","w")
		io.output(file)
...
--	send_msg("InventoryBot","/avventura",ok_cb,false)
	...
	tmp2=io.popen('python3 VillageGameAutoplay_cp/village_game.py "'..tmp..'"'):read()
...
--      	print(msg.text)
	msg.text=string.gsub(msg.text, "'", "#")
      	r=string.format("%s",io.popen('python3 VillageGameAutoplay_cp/setting.py "'..string.format("%s",msg.text)..'"'):read())
      	if ( r ~= "nil" ) then
